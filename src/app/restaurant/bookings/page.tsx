  ...
  return (
    <div className="flex flex-col space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Bookings Manager</h1>
          <p className="text-slate-500 mt-1 text-sm md:text-base">Manage reservations, walk-ins, and floor statuses.</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 border border-slate-200 rounded-md text-sm font-medium bg-white focus:outline-none focus:border-emerald-500 transition-colors h-10 w-full md:w-auto"
            />
          </div>
          <Button onClick={handleAddWalkIn} className="bg-[#1A3636] hover:bg-[#1A3636]/90 text-white gap-2 h-10 whitespace-nowrap">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
        {/* Toolbar */}
        <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 justify-between md:items-center bg-slate-50">
          <div className="flex gap-1 bg-slate-200/50 p-1 rounded-lg overflow-x-auto no-scrollbar w-full md:w-auto">
            {["Upcoming", "Seated", "Completed", "Cancelled"].map(tab => (
              <button 
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-4 py-1.5 rounded-md text-sm font-semibold whitespace-nowrap transition-all ${activeTab === tab ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
              >
                {tab}
              </button>
            ))}
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <Button variant="outline" size="icon" className="bg-white shrink-0"><Filter className="w-4 h-4" /></Button>
          </div>
        </div>
